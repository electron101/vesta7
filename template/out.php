...
?>                
                <div class="btn-group btn-group-justified">
                    <a class="btn btn-default btn-sm btn-flat active " role="button" href="?act=in"><i class="fa fa-download"></i> Входящие <span id="label_list_in">(<?=$row_vhod["count"]?>) </span></a>
                    
                    <a class="btn btn-default btn-sm btn-flat " role="button" id="link_out" href="?act=out"><i class="fa fa-upload"></i> Исходящие <span id="label_list_out">(<?=$row_ishod["count"]?>)</span> </a>
                </div>
...
switch ($_POST["sort_ticket"])
{
    ...
    $result = $mysqli->query("SELECT tickets.id AS id, tickets.prioritet AS prioritet, tickets.theme AS theme, tickets.msg AS msg, clients.fio AS client, tickets.date_create AS date_create, users.fio AS user, tickets.status AS status FROM tickets JOIN clients ON tickets.id_client = clients.id JOIN users ON tickets.user_create_id = users.id WHERE arch = 0 AND tickets.user_create_id = '$id_user'".$pref." ORDER BY tickets.id DESC");
    if ($result):?>
